...
{
    return ethBunch * (DATA_PACKET_HEADER_SIZE +
...
public:
    ...
    BufferData(const BufferData &other) : buffer_size(0), raw_data(nullptr)
    {
        sync_frame = other.sync_frame;
        rms_frame  = other.rms_frame;
        resize(other.buffer_size);
        memcpy(raw_data, other.raw_data, buffer_size * sizeof(unsigned short));
    }
